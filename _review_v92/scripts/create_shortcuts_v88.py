...
def main() -> int:
    ...
    try:
        ...
        print("[v90][WARN] pywin32 is not installed; shortcuts can be created by Inno Setup instead.")
        return 0
        ...
        try:
            ...
            for name, target in targets.items():
                if not target.exists():
                    print(f"[v90][WARN] Missing shortcut target: {target}")
                    continue
                lnk = desk / f"{name}.lnk"
            ...
            print(f"[v90][WARN] Could not create shortcuts in {desk}: {e}")
    print(f"[v90] Created shortcut count: {count}")
    return 0
