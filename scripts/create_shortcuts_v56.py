...
APP_TITLE = "Kakao Emoticon Profit System v56"
...
def make_lnk(target_lnk: Path, batch_file: Path, workdir: Path, description: str) -> bool:
    ...
    vbs = target_lnk.parent / ("_make_shortcut_v56_" + str(os.getpid()) + ".vbs")
    lines = [
...
def main() -> int:
    ...
    for name, bat in targets.items():
        if bat.exists():
            ...
            if make_lnk(lnk, bat, app_dir, f"{APP_TITLE} {name}"):
                report["created"].append(str(lnk))
    (app_dir / "v56_shortcut_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0
