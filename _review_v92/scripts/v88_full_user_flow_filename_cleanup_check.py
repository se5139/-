...
def run_cleanup_fake_flow_test(root: Path) -> dict[str, Any]:
    ...
    current88 = fake_root / "kakao_emoticon_profit_system_v90_current"
    for folder in [old66, old80, current88]:
    ...
    preview = cleanup_run(
        ...
        current_version=88,
        current_path=current88,
        output_root=out_root,
        confirm=None,
        yes=False,
    )
    preview_detected = preview.get("candidate_count") == 2
    ...
    quarantine_moved = quarantine.get("summary", {}).get("acted") == 2 and not old66.exists() and not old80.exists() and current88.exists()
    ...
    return {
        "preview_detected_two_old_versions": preview_detected,
...
def main() -> int:
    root = ROOT
    out = root / "outputs" / "v90_full_user_flow_filename_cleanup_check"
    ...
    checks = {
        ...
        "cleanup_preview_detects_old_versions": cleanup_flow.get("preview_detected_two_old_versions", False),
    ...
    report_path = root / "v90_full_user_flow_filename_cleanup_check_report.json"
