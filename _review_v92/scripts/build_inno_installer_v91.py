...
APP_EXE = "KakaoEmoticonSetup_v91.exe"
SCRIPT_NAME = "KakaoEmoticonSetup_v91.iss"
REPORT_NAME = "v91_inno_build_report.json"
...
def find_inno_compilers() -> list[Path]:
    say("[v91] Searching for Inno Setup compiler...")
    env=os.environ
    candidates=[]

    say("[v91] Checking environment variables and PATH...")
    ...
    say("[v91] Checking standard install folders...")
    roots=[]
    ...
    say("[v91] Checking known desktop/start-menu shortcut target folders only, without recursive scan...")
    ...
    say(f"[v91] Compiler candidates found: {len(valid)}")
    for v in valid[:8]:
        say(f"  - {v}")
    return valid
...
def stream_process(cmd: list[str], cwd: Path, timeout: int=900) -> tuple[int, str]:
    say("[v91] Running compiler command:")
    say("[v91] " + " ".join('"'+x+'"' if ' ' in x else x for x in cmd))
        ...
        while True:
            ...
            if line:
                ...
                if time.time()-last > 15:
                    say("[v91] Still working... waiting for compiler output.")
                    last=time.time()
...
def build(root: Path, iss: Path) -> dict:
    ...
    say("============================================================")
    say("[v91] Kakao Emoticon installer build started.")
    say("[v91] This is a STEP-BY-STEP visible build. It should not look frozen.")
    say("============================================================")
    say(f"[v91] Root: {root}")
    say(f"[v91] Inno script: {iss}")
    if not root.exists():
        report["errors"].append("Root folder does not exist."); say("[v91][ERROR] Root folder does not exist."); return report
    if not iss.exists():
        report["errors"].append("Inno .iss file does not exist."); say("[v91][ERROR] Inno .iss file does not exist."); return report
    ...
    if not compilers:
        msg="Inno Setup compiler was not found. Use _advanced_tools/advanced_bat/OPEN_V91_INNO_SCRIPT.bat or install Inno Setup 6 with command-line compiler."
        report["errors"].append(msg); say("[v91][ERROR] " + msg); return report
    for compiler in compilers:
        say("------------------------------------------------------------")
        say(f"[v91] Trying compiler: {compiler}")
        ...
        try:
            ...
            say(f"[v91] Compiler return code: {code}")
            exe=out_dir/APP_EXE
            if code==0 and exe.exists() and exe.stat().st_size>0:
                report.update({"ok": True, "output": str(exe), "output_size": exe.stat().st_size})
                say("[v91] Installer EXE created successfully.")
                say(f"[v91] Output: {exe}")
                return report
            else:
                say("[v91] Output EXE was not found after this compiler attempt.")
        except Exception as e:
            report["last_result"]={"exception": repr(e)}
            say(f"[v91][ERROR] Exception while running compiler: {e!r}")
    report["errors"].append("Build failed with all detected compiler candidates.")
    return report

def main(argv: list[str] | None = None) -> int:
    ap=argparse.ArgumentParser(description="Build the Kakao Emoticon v91 Windows installer with visible step logs.")
    ...
    say(f"[v91] Build report: {report_path}")
    if not report.get("ok"):
        say("")
        say("[v91] Build failed or compiler not found.")
        say("[v91] Next actions:")
        say("  1. Run _advanced_tools/advanced_bat/OPEN_V91_INNO_SCRIPT.bat and click Build > Compile.")
        say("  2. Or install Inno Setup 6 and run this BAT again.")
        say("  3. Send installer\\v91_inno_build_report.json if it fails again.")
        return 1
    return 0
