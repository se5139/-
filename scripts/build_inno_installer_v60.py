...
APP_EXE = "KakaoEmoticonSetup_v60.exe"
SCRIPT_NAME = "KakaoEmoticonSetup_v60.iss"
...
def main(argv: list[str] | None = None) -> int:
    ap=argparse.ArgumentParser(description="Build the Kakao Emoticon v60 Windows installer. Arguments are optional; paths are inferred when omitted.")
    ...
    report_path=iss.parent/"v60_inno_build_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    if not report.get("ok"):
        print("\n[v60] Build failed or compiler not found.")
        print("[v60] Fix 1: install Inno Setup 6 including command-line compiler.")
        print("[v60] Fix 2: run OPEN_V60_INNO_SCRIPT.bat and compile from the Inno Setup window.")
        print("[v60] Fix 3: set ISCC_PATH to the full path of ISCC.exe, then run this again.")
        return 1
    print("\n[v60] Installer EXE created successfully.")
    print(f"[v60] Output: {report.get('output')}")
    return 0
