...
Write-Report 'Kakao Emoticon v51 old-version cleanup report'
...
if ($dirCandidates.Count -eq 0 -and $linkCandidates.Count -eq 0) {
  ...
  Write-Host '[v51] No old versions found.'
  Write-Host "[v51] Report: $reportPath"
  exit 0
}
...
if ($Mode -eq 'ReportOnly') {
  ...
  Write-Host "[v51] Report only. No files deleted: $reportPath"
  exit 0
}
...
Write-Host "[v51] Cleanup completed. Report: $reportPath"
exit 0
