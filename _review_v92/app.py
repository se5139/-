...
from modules.direct_illustration_studio import DirectIllustrationStudio
...
from modules.system_safety.error_logging import ActionLogger, ErrorHandler, PerformanceLogger, export_diagnostic_package, get_system_diagnostic_summary, mask_sensitive_text
...
ActionLogger.log("v92_app_loaded", {"base_output": str(BASE_OUTPUT)})
...
def init_state() -> None:
    defaults = {
        ...
        "direct_illustration_report": None,
        "workbench_snapshot_path": None,
        "workbench_quick_check_report": None,
        "workbench_one_page_summary": None,
    }
...
st.markdown("""
<style>
:root { --v49-blue:#3563ff; --v50-green:#10b981; --v49-bg:#f5f7fb; --v49-card:#ffffff; --v49-ink:#1f2937; }
...
.v49-hero { background: linear-gradient(135deg, #1f2937 0%, #3563ff 55%, #6d5dfc 100%); padding: 28px 30px; border-radius: 26px; color: white; box-shadow: 0 18px 45px rgba(31,41,55,.18); margin-bottom: 18px; }
.v49-hero h2 { margin: 0; font-size: 30px; }
.v49-hero p { margin: 10px 0 0 0; color: #e5edff; font-size: 16px; }
.v49-card { background: #fff; border: 1px solid #e6eaf2; border-radius: 22px; padding: 20px; box-shadow: 0 10px 26px rgba(15,23,42,.06); }
.v49-step { background: #ffffff; border: 1px solid #e6eaf2; border-radius: 18px; padding: 14px 16px; margin: 8px 0; box-shadow: 0 6px 14px rgba(15,23,42,.05); }
.v49-step b { color: #2549d8; }
div.stButton > button[kind="primary"] { border-radius: 14px; font-weight: 800; padding: .65rem 1.1rem; }
div.stDownloadButton > button { border-radius: 14px; font-weight: 700; }
...
[data-testid="stSidebar"] [role="radiogroup"] label:hover { background: rgba(255,255,255,.16); }
.v57-hero { background: linear-gradient(135deg, #0f172a 0%, #1d4ed8 58%, #06b6d4 100%); color: white; border-radius: 24px; padding: 22px 24px; margin: 10px 0 18px 0; box-shadow: 0 14px 34px rgba(15,23,42,.18); }
.v57-hero h3 { margin: 0 0 6px 0; font-size: 24px; }
.v57-hero p { margin: 0; color: #dbeafe; }
.v57-status-card { background: #ffffff; border: 1px solid #e5e7eb; border-radius: 18px; padding: 14px 16px; box-shadow: 0 8px 22px rgba(15,23,42,.06); margin-bottom: 12px; }

</style>
""", unsafe_allow_html=True)

st.title(f"{APP_NAME} 💬")
st.caption(f"v{APP_VERSION} · 최종 통합 납품 · 재생성→QC→자동보정→수동승인 · Python 로컬 실행")
...
with st.sidebar:
    ...
    st.markdown(
        ...
        11. 저작권/상표권 방어 센터
        12. 설치/실행 진단
        13. 직접 창작 기준/AI 정책 대응
        14. 초보자 직접 캐릭터 만들기 / 최대 5개 소재·첨부파일 생성 / 색상·성격·말투 후보 / 미리보기
        15. 후보 갤러리 / 24개·32개 세트 자동 선택 / 표정 자동 구성
        16. 표정·파츠·문구·움직임 편집기
        17. 카카오톡 채팅창 미리보기/최종 검수
        18. 첫 실제 샘플 세트 제작
        19. 데이터 보호/백업/마이그레이션
        20. 누적 데이터 성장형 추천 엔진
        21. 초보자 전체 제작 마법사
        22. 직접 그리기 캔버스/레이어 편집기
        23. 캐릭터 일관성 검사/자동 보정
        24. 자유 드로잉 캔버스 강화
        25. 자유 드로잉 자동 정리/파츠 추정/표정 확장
        26. 감정 하위표현/행동모션 확장
        27. 텍스트 설명 기반 이모티콘 초안 생성
        28. 누락 정보 후보 제안/재구성
        29. 반려 사유 기반 자동 개선
        30. 제출 전 잠금 체크리스트
        31. 최종 제출 패키지 생성 마법사
        32. 유튜브 참고영상/자막 분석기
        33. 지역 특성/사투리 실생활 문구 엔진
        34. 구체 콘셉트/멘트/모션 템플릿 전략 엔진
        35. 취향/경험 기반 아이디어 발굴·모션 템플릿 고도화
        36. 카카오 규격/용량 최종 검수
        37. 1차 포맷 추천/단계별 확장 전략
        38. CSV/캡처 데이터 가져오기
        39. 카카오 스튜디오 엑셀 성과 학습
        40. 성과 대시보드/다음 제작 방향 추천
        41. 선택된 1차 포맷 자동 변환/압축
        42. 선택 포맷 자동 변환/압축
        43. 플랫폼별 재패키징
        44. 설치형 안정화/오류 진단
        45. v44 전체 통합 검증/최종 납품 패키지
        46. v48 ASCII 안전 설치/이전버전 정리 · v48 ASCII 안전 설치/이전버전 정리
        47. 후보 선택 즉시 적용/정지형 품질 진화
        48. 후보 적용/정지형/움직이는형 품질 진화
        49. 무료 API 수집 안전모드
        50. 리포트/프로젝트 저장
        51. 카카오형 GIF 미리보기/트렌드 개선
        52. Jinja2 템플릿 리포트/프롬프트 엔진
        53. 템플릿 엔진 관리/분리 구조
        54. 실제 제작 품질 개선/영상 기준 반영
        55. 지속 진화형 품질개선/온라인 추상 분석
        56. 실제 그림체/모션 품질 고도화
        57. 세트 완성도 강화/24·32 구성
        58. 제출 전 규격/용량/프레임 QC
        59. 제출 패키지 자동보정/잠금
        60. 제출 전 최종 사용자 확인/수동 승인
        61. 반려 대비/재제출 개선 루프
        62. 캡처 이미지 반려 사유 입력/OCR 보조
        63. 캡처 반려 사유→실제 재생성 자동 연결
        64. 최종 통합 납품/재검사
        """
    )
    st.divider()
    st.warning(
        "이 프로그램은 제작 보조/위험도 검토 도구입니다. 카카오 승인·수익을 보장하지 않으며, 제출 전 공식 가이드를 다시 확인해야 합니다."
    )
...
PAGE_LABELS.extend([
    '65 직접 일러스트 스튜디오/창작 증빙',
    '66 작업대 자동저장/빠른 재검사/요약 리포트',
])
...
COMPACT_WORKFLOWS.insert(0, {
    "label": "AI 자동완성 제작 · 러프→완성 세트",
    "old_index": 64,
    "covers": "신규 65 · 원/몸통/팔다리 러프 → 완성형 PNG/GIF 세트 + 창작 증빙 로그",
    "goal": "사용자가 대충 그린 형태를 입력값으로 삼아 프로그램이 완성형 이모티콘 세트를 생성합니다.",
})
...
if selected_page_index == 13:
    st.subheader("초보자 직접 캐릭터 제작 보조 모드")
    st.write("동그라미 얼굴·몸통·눈·입·기본색·성격·말투처럼 사용자가 직접 정한 형태를 바탕으로, 프로그램이 표현 후보·움직임 계획·창작 증거를 정리합니다.")
    st.info("카카오 심사에서 생성형 AI 완성 이미지는 거부될 수 있으므로, 이 기능은 AI 완성본 생성이 아니라 직접 스케치/도형 입력을 정리하고 확장하는 제작 보조 모드입니다.")
    ...
    st.warning("첨부 이미지는 그대로 합성/복제하지 않고, 색감·형태·비율 힌트와 창작 증거로만 사용합니다. 직접 창작 게이트를 통과하지 못하면 결과물은 제출 후보가 아니라 보완용 초안입니다.")

    with st.expander("직접 창작 게이트 · 제출 전 필수 확인", expanded=True):
        gate_c1, gate_c2 = st.columns(2)
        with gate_c1:
            multi_has_sketch = st.checkbox("손스케치/도형 러프 또는 직접 만든 참고자료를 첨부했습니다", value=False, key="multi_has_sketch_gate")
            multi_has_revision_plan = st.checkbox("몸통/눈/입/팔/말풍선/효과 레이어 또는 수정 이력을 남길 계획입니다", value=False, key="multi_revision_gate")
        with gate_c2:
            multi_rights_confirmed = st.checkbox("유명 캐릭터·브랜드·작가 스타일·타인 이미지를 사용하지 않았습니다", value=False, key="multi_rights_gate")
            multi_uses_ai_completed = st.checkbox("AI가 만든 완성 이미지를 제출 후보로 사용합니다", value=False, key="multi_ai_completed_gate")
        multi_creator_note = st.text_area(
            "직접 창작 메모",
            value="내가 정한 소재·도형·색상·성격·말투를 바탕으로 러프를 만들고, 프로그램은 문구/모션/QC/증거 정리만 보조합니다.",
            height=90,
            key="multi_creator_note_gate",
        )
        if multi_uses_ai_completed:
            st.error("AI 완성 이미지는 제출 후보로 쓰지 않는 방향이 안전합니다. 직접 스케치/레이어 기반으로 다시 제작하세요.")
    ...
    if run_multi:
        try:
            report = MultiMaterialCharacterCreator().build_project(
                ...
                creator_note=multi_creator_note,
                has_layer_or_revision_plan=multi_has_revision_plan,
                rights_confirmed=multi_rights_confirmed,
                uses_ai_completed_image=multi_uses_ai_completed,
            )
    ...
    if st.session_state.multi_material_creator_report:
        ...
        gate = mreport.get("direct_creation_gate", {})
        if gate:
            st.markdown("### 직접 창작 게이트")
            g1, g2, g3 = st.columns(3)
            g1.metric("상태", gate.get("status"))
            g2.metric("점수", gate.get("score"))
            g3.metric("통과", "YES" if gate.get("passed") else "NO")
    ...
    with st.expander("듀오 직접 창작 게이트", expanded=False):
        duo_has_sketch = st.checkbox("듀오 손스케치/도형 러프를 준비했습니다", value=False, key="duo_has_sketch_gate")
        duo_has_revision_plan = st.checkbox("듀오 레이어/수정 이력을 남길 계획입니다", value=False, key="duo_revision_gate")
        duo_rights_confirmed = st.checkbox("듀오 캐릭터가 타인 저작권/상표권/유명 캐릭터를 사용하지 않습니다", value=False, key="duo_rights_gate")
        duo_uses_ai_completed = st.checkbox("듀오 AI 완성 이미지를 제출 후보로 사용합니다", value=False, key="duo_ai_completed_gate")
        if duo_uses_ai_completed:
            st.error("AI 완성 이미지는 제출 후보에서 제외하고 직접 만든 레이어/스케치 기반으로 다시 제작하세요.")
    ...
    if st.button("초보자 직접 캐릭터 프로젝트 생성", type="primary"):
        try:
            spec = DuoCharacterInput(
                ...
                has_hand_sketch=duo_has_sketch,
                has_layer_or_revision_plan=duo_has_revision_plan,
                rights_confirmed=duo_rights_confirmed,
                uses_ai_completed_image=duo_uses_ai_completed,
            )
    ...
    if st.session_state.beginner_creator_report:
        ...
        gate = report.get("direct_creation_gate", {})
        if gate:
            st.markdown("### 직접 창작 게이트")
            g1, g2, g3 = st.columns(3)
            g1.metric("상태", gate.get("status"))
...
if selected_page_index == 56:
    ...
    if st.button("v70 세트 완성도 패키지 생성", type="primary", key="v70_run_set_completion"):
        ActionLogger.log("v92_ui_clicked_set_completion", {"project_name": v70_project_name, "style": v70_style})
        try:
            _v70_start = __import__("time").perf_counter()
            ...
            PerformanceLogger.log("v92_ui_set_completion", __import__("time").perf_counter() - _v70_start, "PASS", {"output_dir": result_obj.output_dir})
            st.success("v70 정지형 32개/움직이는형 24개 세트 후보와 제출 후보 패키지를 생성했습니다.")
        except Exception as exc:
            error_id = ErrorHandler.log_error(exc, "v70_set_completion_ui", {"project_name": v70_project_name})
            st.error(f"v70 세트 완성도 생성 실패 · 오류 ID: {error_id}")
            with st.expander("고급 기술 정보 보기", expanded=False):
                st.code(mask_sensitive_text(exc), language="text")
...
if selected_page_index == 63:
    ...
    with st.expander("v92 이전 버전 안전 정리 · C드라이브/설치 폴더 청소", expanded=False):
        st.write("C드라이브 루트나 LocalAppData에 남은 이전 버전 폴더를 검사합니다. v92 설치 흐름에서는 이전 버전 백업 후 정리를 자동으로 실행하고, 이 화면에서는 먼저 미리보기 리포트를 확인합니다.")
        st.code("00_STEP_4_SAFE_CLEAN_OLD_VERSIONS.bat", language="text")
        st.markdown("- 1번: 검사만 실행\n- 2번: 안전 격리 폴더로 이동\n- 3번: 사용자 데이터 후보 백업 후 영구 삭제")
        if st.button("이전 버전 미리보기 검사 실행", key="v92_cleanup_preview_button"):
            try:
                proc = subprocess.run(
                    [sys.executable, "scripts/cleanup_old_versions_v92.py", "--mode", "preview", "--current-version", "92", "--current-path", str(Path.cwd())],
                    cwd=str(Path.cwd()),
                    capture_output=True,
                    text=True,
                    timeout=180,
        ...
        st.caption("수동 정리/삭제는 BAT 파일에서 확인 문구를 입력해야 진행됩니다. 설치/업그레이드 흐름에서는 현재 v92 이상을 제외하고 v91 이하만 백업 후 정리합니다.")

    with st.expander("v92 안전 오류진단 · 로그/성능 리포트", expanded=False):
        st.write("오류 ID, 사용자 흐름 로그, 성능 로그를 사용자 데이터 폴더에 저장합니다. API 키/비밀번호 형태 값은 저장 전 마스킹합니다.")
        diag = get_system_diagnostic_summary(limit=8)
        st.markdown("#### 저장 위치")
        st.code(diag.get("log_dir", ""), language="text")
        fcols = st.columns(4)
        for idx, (name, meta) in enumerate(diag.get("files", {}).items()):
            with fcols[idx % 4]:
                st.metric(name, meta.get("size_bytes", 0))
        x, y, z = st.columns(3)
        with x:
            st.markdown("#### 최근 오류")
            st.json(diag.get("recent_errors", []))
        with y:
            st.markdown("#### 최근 작업")
            st.json(diag.get("recent_actions", []))
        with z:
            st.markdown("#### 최근 성능")
            st.json(diag.get("recent_performance", []))
        if st.button("v92 진단 테스트 로그 1건 기록", key="v92_diagnostic_test_log"):
            ActionLogger.log("v92_manual_diagnostic_test", {"screen": "final_delivery", "note": "manual test"})
            PerformanceLogger.log("v92_manual_diagnostic_test", 0.0, "PASS")
            st.success("진단 테스트 로그를 기록했습니다.")
        if st.button("v92 오류/성능 진단 ZIP 내보내기", key="v92_export_diagnostic_zip"):
            try:
                zip_path = export_diagnostic_package()
                st.success(f"진단 ZIP 생성 완료: {zip_path}")
                st.download_button("진단 ZIP 다운로드", data=Path(zip_path).read_bytes(), file_name=Path(zip_path).name, mime="application/zip", key="v92_diagnostic_zip_download")
            except Exception as exc:
                error_id = ErrorHandler.log_error(exc, "v92_export_diagnostic_zip")
                st.error(f"진단 ZIP 생성 실패 · 오류 ID: {error_id}")
        st.caption("로그는 문제 해결용입니다. 원문 API 키/비밀번호 저장 금지, 코드 폴더와 사용자 데이터 폴더 분리 원칙을 적용합니다.")
...
if selected_page_index == 64:
    st.subheader("AI 자동완성 제작 / 러프 스케치 → 완성형 이모티콘 세트")
    st.write("사용자가 대충 그린 원, 몸통, 팔다리 또는 자유 러프를 입력값으로 삼아 프로그램이 32개 PNG와 24개 GIF 후보를 자동 생성합니다.")
    st.info("핵심 구조: 1 주기능=러프 도형 입력 후 자동완성, 2 부기능=자유 러프 기반 자동완성, 3 추가 부기능=직접 일러스트 편집/창작 그림 포함.")
    st.warning("유명 캐릭터, 브랜드, 타 작가 그림체, 인터넷 이미지 복제는 피해야 합니다. 러프와 생성 로그는 창작 기록 자료로 함께 저장됩니다.")

    left, right = st.columns([1.08, 0.92])
    with left:
        studio_project = st.text_input("프로젝트명", value="direct_illustration_project", key="direct_illustration_project")
        studio_concept = st.text_area(
            "캐릭터 콘셉트 / 내가 직접 만든 설정",
            value="조금 허둥대지만 성실한 작은 메모지 캐릭터. 직장 대화에서 짧고 귀엽게 반응한다.",
            height=100,
            key="direct_illustration_concept",
        )
        studio_creator_note = st.text_area(
            "창작 기록 메모",
            value="내가 대충 원, 몸통, 팔다리 러프를 그리고 프로그램이 이를 바탕으로 완성형 이모티콘 세트를 생성한다.",
            height=105,
            key="direct_illustration_creator_note",
        )
        generation_mode = st.radio(
            "생성 모드",
            ["주기능: 원/몸통/팔다리 러프 자동완성", "부기능: 자유 러프 자동완성", "추가 부기능: 일러스트 편집용 직접 그림 포함"],
            horizontal=False,
            key="direct_illustration_generation_mode",
        )
        rough_upload = st.file_uploader("대충 그린 러프 스케치 PNG/JPG", type=["png", "jpg", "jpeg", "webp"], key="direct_illustration_rough")
        shape = st.selectbox("러프 해석 기본 몸통", ["round", "square", "capsule"], format_func=lambda x: {"round": "둥근 원형", "square": "몸통 사각형", "capsule": "길쭉한 몸통"}[x], key="direct_illustration_shape")
        c1, c2 = st.columns(2)
        base_color = c1.color_picker("기본색", "#F2C078", key="direct_illustration_base_color")
        accent_color = c2.color_picker("포인트색", "#F49BAB", key="direct_illustration_accent_color")
        rights_confirmed = st.checkbox("저작권/상표권/유명 캐릭터/타 작가 그림체를 사용하지 않았음을 확인", value=False, key="direct_illustration_rights")
        no_ai_completed = st.checkbox("러프 입력 기반 자동완성 생성 결과를 만들고, 창작 기록을 함께 보관", value=True, key="direct_illustration_no_ai")

    with right:
        st.markdown("### 기능 분리")
        st.markdown(
            """
            - 주기능: 원/몸통/팔다리만 대충 그려도 완성형 캐릭터 세트 생성
            - 부기능: 그리고 싶은 형태를 자유롭게 대충 그리면 그 형태를 토대로 생성
            - 추가 부기능: 직접 일러스트용 결과물과 창작 그림/러프 원본을 함께 보관
            - 생성 결과: 32개 PNG, 24개 GIF, 컨택트시트, 리포트, 창작 로그 ZIP
            - 학습/자료수집 결과는 복제용이 아니라 방향 참고 메모로만 연결
            """
        )
        learning_hint = ""
        if st.session_state.get("evolution_learning_report"):
            learning_hint = json.dumps(st.session_state.get("evolution_learning_report"), ensure_ascii=False)[:1200]
            st.success("학습형 자료 수집 요약이 있어 추상 참고 메모로 연결할 수 있습니다.")
        else:
            st.info("학습형 자료 수집을 먼저 실행하지 않아도 제작은 가능합니다.")
        learning_summary = st.text_area("학습/분석 참고 요약", value=learning_hint, height=150, key="direct_illustration_learning_summary")

    if st.button("러프 기반 완성형 PNG/GIF 세트 자동 생성", type="primary", key="run_direct_illustration_studio"):
        try:
            sketch_path = None
            if rough_upload is not None:
                temp_dir = BASE_OUTPUT / "direct_illustration_uploads"
                temp_dir.mkdir(parents=True, exist_ok=True)
                sketch_path = temp_dir / rough_upload.name
                sketch_path.write_bytes(rough_upload.getvalue())
            report = DirectIllustrationStudio().build_project(
                output_dir=BASE_OUTPUT / "direct_illustration_studio",
                project_name=studio_project,
                concept_note=studio_concept,
                creator_note=studio_creator_note,
                sketch_path=sketch_path,
                base_color=base_color,
                accent_color=accent_color,
                body_shape=shape,
                rights_confirmed=rights_confirmed,
                no_ai_completed_image=not no_ai_completed,
                learning_summary=learning_summary,
                generation_mode=generation_mode,
            )
            st.session_state.direct_illustration_report = report.to_dict()
            st.success("러프 기반 완성형 PNG/GIF 세트와 창작 증빙 패키지를 생성했습니다.")
        except Exception as exc:
            st.error(f"직접 일러스트 스튜디오 실행 실패: {exc}")

    studio_report = st.session_state.get("direct_illustration_report")
    if studio_report:
        st.markdown("### 생성 결과")
        m1, m2, m3 = st.columns(3)
        m1.metric("상태", studio_report.get("status"))
        m2.metric("창작/생성 점수", studio_report.get("score"))
        m3.metric("PNG/GIF", f"{studio_report.get('static_count', 0)} / {studio_report.get('animated_count', 0)}")
        if studio_report.get("warnings"):
            for warning in studio_report.get("warnings", []):
                st.warning(warning)
        preview = Path(studio_report.get("contact_sheet_path", ""))
        if preview.exists():
            st.image(str(preview), caption="표정 후보 컨택트시트", use_container_width=True)
        for label, key, mime in [
            ("대표 완성 360 PNG 다운로드", "clean_png_path", "image/png"),
            ("정지형 32개 PNG ZIP 다운로드", "static_zip_path", "application/zip"),
            ("움직이는형 24개 GIF ZIP 다운로드", "animated_zip_path", "application/zip"),
            ("창작/생성 증빙 JSON 다운로드", "creator_ledger_path", "application/json"),
            ("자동완성 리포트 HTML 다운로드", "html_path", "text/html"),
            ("전체 작업 패키지 ZIP 다운로드", "zip_path", "application/zip"),
        ]:
            fp = Path(studio_report.get(key, ""))
            if fp.exists():
                st.download_button(label, data=fp.read_bytes(), file_name=fp.name, mime=mime, key=f"direct_illustration_download_{key}")

if selected_page_index == 65:
    st.subheader("작업대 자동저장 / 빠른 재검사 / 한 장 요약 리포트")
    st.write("최근 작업을 저장하고 이어서 만들 수 있게 하며, 제출 전 실패/경고 항목 중심으로 빠르게 재검사합니다.")

    snap_dir = BASE_OUTPUT / "workbench_enhancements"
    snap_path = snap_dir / "recent_project_snapshot.json"
    c1, c2, c3 = st.columns(3)
    if c1.button("최근 작업 자동 저장", type="primary", key="workbench_save_snapshot"):
        try:
            path = save_project_snapshot(st.session_state, snap_dir, APP_VERSION)
            st.session_state.workbench_snapshot_path = str(path)
            st.success(f"저장 완료: {path}")
        except Exception as exc:
            st.error(f"자동 저장 실패: {exc}")
    if c2.button("이어서 만들기 불러오기", key="workbench_load_snapshot"):
        try:
            data = load_project_snapshot(snap_path)
            restored = 0
            for key, value in data.get("state", {}).items():
                st.session_state[key] = value
                restored += 1
            st.success(f"최근 작업 {restored}개 항목을 복원했습니다.")
        except Exception as exc:
            st.error(f"불러오기 실패: {exc}")
    if c3.button("실패/경고 항목만 빠른 재검사", key="workbench_rerun_failed"):
        previous = st.session_state.get("workbench_quick_check_report") or st.session_state.get("v71_pre_submission_qc_report") or {}
        report = rerun_failed_items(previous)
        st.session_state.workbench_quick_check_report = report
        st.info("이전 리포트의 실패/경고 항목만 빠르게 요약했습니다. 실제 파일 재검사는 아래 업로드 검사로 실행하세요.")

    upload = st.file_uploader("PNG/GIF/JPG/WEBP 또는 ZIP 업로드 검사", type=["png", "jpg", "jpeg", "gif", "webp", "zip"], key="workbench_quick_upload")
    if upload is not None and st.button("업로드 파일 검사 실행", type="primary", key="workbench_run_upload_check"):
        try:
            report = analyze_uploaded_package(upload.name, upload.getvalue())
            st.session_state.workbench_quick_check_report = report
            st.success("업로드 파일 검사를 완료했습니다.")
        except Exception as exc:
            st.error(f"업로드 검사 실패: {exc}")

    quick_report = st.session_state.get("workbench_quick_check_report")
    if quick_report:
        st.markdown("### 검사 결과")
        q1, q2, q3, q4 = st.columns(4)
        q1.metric("전체 상태", quick_report.get("overall_status"))
        q2.metric("PASS", quick_report.get("pass_count", 0))
        q3.metric("WARN", quick_report.get("warn_count", 0))
        q4.metric("FAIL", quick_report.get("fail_count", 0))
        st.dataframe(pd.DataFrame(quick_report.get("rows", [])), use_container_width=True)
        if st.button("최종 제출용 한 장 요약 리포트 생성", key="workbench_create_one_page_summary"):
            try:
                path = create_one_page_summary(quick_report, snap_dir, APP_VERSION)
                st.session_state.workbench_one_page_summary = str(path)
                st.success(f"요약 리포트 생성 완료: {path}")
            except Exception as exc:
                st.error(f"요약 리포트 생성 실패: {exc}")
    summary_path = Path(st.session_state.get("workbench_one_page_summary") or "")
    if summary_path.exists():
        st.download_button("한 장 요약 리포트 다운로드", data=summary_path.read_bytes(), file_name=summary_path.name, mime="text/html", key="download_one_page_summary")
