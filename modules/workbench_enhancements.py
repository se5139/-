...
def analyze_image_bytes(name: str, data: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        if width != 360 or height != 360:
            warnings.append(f"권장 크기 360x360과 다릅니다: {width}x{height}")
        if len(data) > 1024 * 1024:
            warnings.append("파일 크기가 1MB를 넘습니다. 제출 전 용량 압축을 확인하세요.")
        if fmt == "GIF" and frame_count < 2:
            warnings.append("GIF인데 프레임이 1개뿐입니다.")
        if contrast < 42:
            warnings.append("전체 대비가 낮아 작은 썸네일에서 흐릿할 수 있습니다.")
        if edge_ratio > 0.18:
            warnings.append("이미지가 가장자리까지 닿아 글자/그림 잘림 가능성이 있습니다.")

        issues.extend(warnings)
        if any("금지" in item or "예약" in item for item in issues):
            status = "FAIL"
        elif issues:
            status = "WARN"
        return {
...
def rerun_failed_items(previous_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "업로드 원본 없이 이전 실패/경고 항목만 다시 요약했습니다. 파일을 다시 올리면 실제 재검사를 수행합니다.",
    }
...
<main>
  ...
  <p>이 리포트는 로컬 사전 점검 결과입니다. 제출 직전 카카오 공식 기준과 저작권/상표권 위험은 다시 확인해야 합니다.</p>
  <table>
