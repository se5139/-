...
APP_EXE = "KakaoEmoticonSetup_v87.exe"
SCRIPT_NAME = "KakaoEmoticonSetup_v87.iss"
REPORT_NAME = "v87_inno_build_report.json"
...
def find_inno_compilers() -> list[Path]:
    say("[v87] Searching for Inno Setup compiler...")
    env=os.environ
    candidates=[]

    say("[v87] Checking environment variables and PATH...")
    ...
    say("[v87] Checking standard install folders...")
    roots=[]
    ...
    say("[v87] Checking known desktop/start-menu shortcut target folders only, without recursive scan...")
    ...
    say(f"[v87] Compiler candidates found: {len(valid)}")
    for v in valid[:8]:
        say(f"  - {v}")
    return valid
...
def stream_process(cmd: list[str], cwd: Path, timeout: int=900) -> tuple[int, str]:
    say("[v87] Running compiler command:")
    say("[v87] " + " ".join('"'+x+'"' if ' ' in x else x for x in cmd))
        ...
        while True:
            ...
            if line:
                ...
                if time.time()-last > 15:
                    say("[v87] Still working... waiting for compiler output.")
                    last=time.time()
...
def build(root: Path, iss: Path) -> dict:
    report={"time": datetime.now().isoformat(timespec="seconds"), "version": "87", "root": str(root), "iss": str(iss), "ok": False, "errors": [], "candidates": []}
    say("============================================================")
    say("[v87] Kakao Emoticon installer build started.")
    say("[v87] This is a STEP-BY-STEP visible build. It should not look frozen.")
    say("============================================================")
    say(f"[v87] Root: {root}")
    say(f"[v87] Inno script: {iss}")
    if not root.exists():
        report["errors"].append("Root folder does not exist."); say("[v87][ERROR] Root folder does not exist."); return report
    if not iss.exists():
        report["errors"].append("Inno .iss file does not exist."); say("[v87][ERROR] Inno .iss file does not exist."); return report
    ...
    if not compilers:
        msg="Inno Setup compiler was not found. Use _advanced_tools/advanced_bat/OPEN_V87_INNO_SCRIPT.bat or install Inno Setup 6 with command-line compiler."
        report["errors"].append(msg); say("[v87][ERROR] " + msg); return report
    for compiler in compilers:
        say("------------------------------------------------------------")
        say(f"[v87] Trying compiler: {compiler}")
        ...
        try:
            ...
            say(f"[v87] Compiler return code: {code}")
            exe=out_dir/APP_EXE
            if code==0 and exe.exists() and exe.stat().st_size>0:
                report.update({"ok": True, "output": str(exe), "output_size": exe.stat().st_size})
                say("[v87] Installer EXE created successfully.")
                say(f"[v87] Output: {exe}")
                return report
            else:
                say("[v87] Output EXE was not found after this compiler attempt.")
        except Exception as e:
            report["last_result"]={"exception": repr(e)}
            say(f"[v87][ERROR] Exception while running compiler: {e!r}")
    report["errors"].append("Build failed with all detected compiler candidates.")
    return report

def main(argv: list[str] | None = None) -> int:
    ap=argparse.ArgumentParser(description="Build the Kakao Emoticon v87 Windows installer with visible step logs.")
    ...
    say(f"[v87] Build report: {report_path}")
    if not report.get("ok"):
        say("")
        say("[v87] Build failed or compiler not found.")
        say("[v87] Next actions:")
        say("  1. Run _advanced_tools/advanced_bat/OPEN_V87_INNO_SCRIPT.bat and click Build > Compile.")
        say("  2. Or install Inno Setup 6 and run this BAT again.")
        say("  3. Send installer\\v87_inno_build_report.json if it fails again.")
        return 1
    return 0
