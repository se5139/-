...
from modules.system_safety.error_logging import ActionLogger, ErrorHandler, PerformanceLogger
...
class V70SetCompletenessReport:
    ...
    v92_simple_output_dir: str = ""
    v92_static_png_submit_dir: str = ""
    v92_animated_gif_submit_dir: str = ""
    v92_preview_jpg_dir: str = ""
    v92_submit_only_png_gif_zip: str = ""
    v92_simple_output_package_zip: str = ""
    v92_manifest_json: str = ""
    v92_manifest_csv: str = ""
    checksum_sha256: str = ""
...
<div class=\"card\"><h2>Identity Lock</h2><div class=\"mono\">{{ identity_pretty }}</div></div>
    ...
    def _save_v92_jpg_preview(self, source_path: Path, out_path: Path) -> None:
    ...
    def _build_v92_simple_output(
        ...
        simple_dir = run_dir / "v92_simple_png_gif_output"
        ...
        for row in static_rows:
            ...
            if src.exists():
                ...
                self._save_v92_jpg_preview(dst, jpg)
        ...
        manifest_json = meta_dir / "v92_simple_output_manifest.json"
        manifest_csv = meta_dir / "v92_simple_output_manifest.csv"
        manifest = {
        ...
        self._write_rows(manifest_csv, meta_dir / "v92_simple_output_manifest_rows.json", manifest_rows or [{"no":"", "kind":"", "submit_file":"", "preview_jpg":"", "original_phrase":"", "emotion":"", "final_format":"", "jpg_rule":""}])

        guide = simple_dir / "README_V92_OUTPUT_KO.txt"
        guide.write_text(
            "v92 간편 출력 구조\n\n"
        ...
        submit_only = run_dir / "v92_submit_only_png_gif.zip"
        with zipfile.ZipFile(submit_only, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            zf.write(manifest_json, "metadata/v92_simple_output_manifest.json")
        simple_zip = run_dir / "v92_simple_output_package.zip"
        self._zip_tree(simple_zip, simple_dir, include_root=True)
        return {
            "v92_simple_output_dir": str(simple_dir),
            "v92_static_png_submit_dir": str(static_submit),
            "v92_animated_gif_submit_dir": str(animated_submit),
            "v92_preview_jpg_dir": str(preview_jpg),
            "v92_submit_only_png_gif_zip": str(submit_only),
            "v92_simple_output_package_zip": str(simple_zip),
            "v92_manifest_json": str(manifest_json),
            "v92_manifest_csv": str(manifest_csv),
        }
    ...
    def build_bundle(
        ...
        _bundle_start = time.perf_counter()
        ActionLogger.log("v92_set_completeness_build_started", {"project_name": project_name, "selected_style": selected_style})
        ...
        static_zip = run_dir / "v92_static_32_png_package.zip"
        animated_zip = run_dir / "v92_animated_24_gif_package.zip"
        submission_zip = run_dir / "v92_submission_candidate_package.zip"
        ...
        v92_paths = self._build_v92_simple_output(run_dir, static_rows, animated_rows, project_name)
        checksum = self._checksum_v70(submission_zip)
        PerformanceLogger.log("v92_set_completeness_build_bundle", time.perf_counter() - _bundle_start, "PASS", {"output_dir": str(run_dir)})
        ActionLogger.log("v92_set_completeness_build_completed", {"output_dir": str(run_dir), "static_count": len(static_rows), "animated_count": len(animated_rows)})
        return V70SetCompletenessReport(
            ...
            v92_simple_output_dir=v92_paths["v92_simple_output_dir"],
            v92_static_png_submit_dir=v92_paths["v92_static_png_submit_dir"],
            v92_animated_gif_submit_dir=v92_paths["v92_animated_gif_submit_dir"],
            v92_preview_jpg_dir=v92_paths["v92_preview_jpg_dir"],
            v92_submit_only_png_gif_zip=v92_paths["v92_submit_only_png_gif_zip"],
            v92_simple_output_package_zip=v92_paths["v92_simple_output_package_zip"],
            v92_manifest_json=v92_paths["v92_manifest_json"],
            v92_manifest_csv=v92_paths["v92_manifest_csv"],
