...
class BeginnerCharacterCreator:
    ...
    def build_project(self, spec: DuoCharacterInput, output_dir: Path, project_name: str, expression_count: int = 80, preview_count: int = 12) -> BeginnerCreationReport:
        ...
        gate = DirectCreationGate().evaluate(
            has_user_shape_input=bool(spec.material_a and spec.material_b and spec.base_shape),
            has_sketch_or_reference=spec.has_hand_sketch,
            has_creator_note=bool(spec.creator_note.strip()),
            has_layer_or_revision_plan=spec.has_layer_or_revision_plan,
            has_rights_confirmation=spec.rights_confirmed,
            uses_ai_completed_image=spec.uses_ai_completed_image,
        ).to_dict()
        ...
        report = BeginnerCreationReport(project_name, spec.to_dict(), assets, expressions, motion, notes, gate, timeline, "", "", "")
