...
class MultiMaterialCharacterCreator:
    ...
    def render_multi_group(self, specs: List[MaterialSpec], out_dir: Path, project_name: str) -> List[Dict[str, Any]]:
        ...
        fp = out_dir / f"{project_name}_multi_material_group.png"
        img.save(fp)
    ...
    def build_project(self, specs: List[MaterialSpec], uploaded_files: List[Any], output_dir: Path, project_name: str, expression_count: int = 80, preview_count: int = 12) -> MultiMaterialReport:
        specs = specs[:5]
        if not specs:
        ...
        assets = self.render_multi_group(specs, root / "assets", project_name)
        expressions = self.build_expression_table(specs, expression_count)
        assets.extend(self.render_expression_previews(specs, expressions, root / "expression_previews", project_name, preview_count))
        ...
        report = MultiMaterialReport(project_name, [s.to_dict() for s in specs], sources, assets, expressions, motion, notes, timeline, "", "", "")
        json_path = root / "multi_material_creator_report.json"
        html_path = root / "multi_material_creator_report.html"
        zip_path = output_dir / f"{project_name}_multi_material_creator_pack.zip"
