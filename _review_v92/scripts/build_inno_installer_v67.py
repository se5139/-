...
APP_EXE = "KakaoEmoticonSetup_v67.exe"
SCRIPT_NAME = "KakaoEmoticonSetup_v67.iss"
...
def main(argv: list[str] | None = None) -> int:
    ap=argparse.ArgumentParser(description="Build the Kakao Emoticon v67 Windows installer. Arguments are optional; paths are inferred when omitted.")
    ...
    report_path=iss.parent/"v67_inno_build_report.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    if not report.get("ok"):
        print("\n[v67] Build failed or compiler not found.")
        print("[v67] Fix 1: install Inno Setup 6 including command-line compiler.")
        print("[v67] Fix 2: run OPEN_V67_INNO_SCRIPT.bat and compile from the Inno Setup window.")
        print("[v67] Fix 3: set ISCC_PATH to the full path of ISCC.exe, then run this again.")
        return 1
    print("\n[v67] Installer EXE created successfully.")
    print(f"[v67] Output: {report.get('output')}")
    return 0
