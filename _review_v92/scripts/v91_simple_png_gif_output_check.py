...
from scripts.cleanup_old_versions_v91 import run as cleanup_run
...
def run_cleanup_fake_flow_test(root: Path) -> dict[str, Any]:
    fake_root = root / "outputs" / "v91_cleanup_fake_scan_root"
    if fake_root.exists():
        import shutil
    ...
    current91 = fake_root / "kakao_emoticon_profit_system_v91_current"
    for folder in [old66, old80, old88, old89, old90, current91]:
    ...
    out_root = root / "outputs" / "v91_cleanup_test_reports"

    preview = cleanup_run(
        mode="preview",
        scan_roots=[fake_root],
        current_version=91,
        current_path=current91,
        output_root=out_root,
        confirm=None,
        yes=False,
    )
    preview_detected = preview.get("candidate_count") == 5
    current_excluded = all("v91_current" not in c.get("path", "") for c in preview.get("candidates", []))
    ...
    quarantine_moved = quarantine.get("summary", {}).get("acted") == 5 and not old66.exists() and not old80.exists() and not old88.exists() and not old89.exists() and not old90.exists() and current91.exists()
    ...
    return {
        "preview_detected_five_old_versions_including_v90": preview_detected,
        "current_v91_excluded": current_excluded,
...
def main() -> int:
    root = ROOT
    out = root / "outputs" / "v91_simple_png_gif_output_check"
    ...
    report = eng.build_bundle(
        project_name="v91_초보자_전체흐름_파일명안전_점검",
    ...
    static_submit_files = sorted(Path(report.v91_static_png_submit_dir).glob("*.png"))
    animated_submit_files = sorted(Path(report.v91_animated_gif_submit_dir).glob("*.gif"))
    preview_jpg_files = sorted(Path(report.v91_preview_jpg_dir).glob("*.jpg"))
    simple_manifest = read_json(Path(report.v91_manifest_json))
    simple_manifest_csv_rows = read_csv_rows(Path(report.v91_manifest_csv))
    ...
    zip_invalid = {
        ...
        "v91_submit_only_png_gif_zip": check_zip_names(Path(report.v91_submit_only_png_gif_zip)),
        "v91_simple_output_package_zip": check_zip_names(Path(report.v91_simple_output_package_zip)),
    }
    with zipfile.ZipFile(report.v91_submit_only_png_gif_zip) as zf:
    ...
    checks = {
        ...
        "v91_static_png_submit_32": len(static_submit_files) == 32,
        "v91_animated_gif_submit_24": len(animated_submit_files) == 24,
        "v91_preview_jpg_56_created": len(preview_jpg_files) == 56,
        "v91_submit_only_zip_has_no_jpg": not submit_only_jpg_entries,
        "v91_manifest_counts_match": (
        ...
        "cleanup_preview_detects_old_versions_including_v90": cleanup_flow.get("preview_detected_five_old_versions_including_v90", False),
        "cleanup_excludes_current_v91": cleanup_flow.get("current_v91_excluded", False),
    ...
    summary = {
        ...
        "counts": {
            ...
            "v91_static_png_submit_count": len(static_submit_files),
            "v91_animated_gif_submit_count": len(animated_submit_files),
            "v91_preview_jpg_count": len(preview_jpg_files),
            "v91_manifest_csv_row_count": len(simple_manifest_csv_rows),
        ...
        "v91_submit_only_png_gif_zip": report.v91_submit_only_png_gif_zip,
        "v91_submit_only_png_gif_zip_sha256": sha256_file(Path(report.v91_submit_only_png_gif_zip)),
        "v91_simple_output_package_zip": report.v91_simple_output_package_zip,
        "v91_simple_output_package_zip_sha256": sha256_file(Path(report.v91_simple_output_package_zip)),
    }

    report_path = root / "v91_simple_png_gif_output_check_report.json"
