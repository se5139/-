...
APP_EXE = "KakaoEmoticonSetup_v58.exe"
...
def main() -> int:
    ...
    report_path=iss.parent/"v58_inno_build_report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    if not report.get("ok"):
        print("\n[v58] Build failed or compiler not found.")
        print("[v58] Fix 1: install Inno Setup 6 including command-line compiler.")
        print("[v58] Fix 2: run OPEN_V58_INNO_SCRIPT.bat and compile from the Inno Setup window.")
        print("[v58] Fix 3: set ISCC_PATH to the full path of ISCC.exe, then run this again.")
        return 1
    return 0
