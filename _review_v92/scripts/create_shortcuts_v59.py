...
APP_TITLE = "Kakao Emoticon Profit System v59"
...
def make_lnk(target_lnk: Path, batch_file: Path, workdir: Path, description: str) -> bool:
    ...
    vbs = target_lnk.parent / ("_make_shortcut_v59_" + str(os.getpid()) + ".vbs")
    lines = [
...
def main() -> int:
    ...
    targets = {
        ...
        "Clean Old Versions": app_dir / "14_V59_CLEAN_OLD_VERSIONS_AND_EXTRACTED.bat",
    ...
    for name, bat in targets.items():
        if bat.exists():
            ...
            if make_lnk(lnk, bat, app_dir, f"{APP_TITLE} {name}"):
                report["created"].append(str(lnk))
    (app_dir / "v59_shortcut_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps(report, ensure_ascii=False, indent=2))
    return 0
