...
cd /d "%~dp0"
title Kakao Emoticon v92 - Safe Old Version Cleanup

echo.
echo ============================================================
echo  Kakao Emoticon Profit System v92 - Safe Old Version Cleanup
echo ============================================================
echo [v92] This tool scans old Kakao emoticon program folders such as:
...
echo [SAFETY] Current version v92 is excluded.
...
:PREVIEW
echo.
echo [v92] Running preview only...
%PY_CMD% scripts\cleanup_old_versions_v92.py --mode preview --current-version 92 --current-path "%CD%"
goto DONE

:QUARANTINE
echo.
echo [v92] Quarantine mode moves old version folders into a backup/quarantine folder.
echo [v92] This is safer than permanent deletion because you can still inspect them.
echo.
...
%PY_CMD% scripts\cleanup_old_versions_v92.py --mode quarantine --yes --confirm MOVE_OLD_KAKAO_VERSIONS --current-version 92 --current-path "%CD%"
goto DONE
...
%PY_CMD% scripts\cleanup_old_versions_v92.py --mode delete --yes --confirm DELETE_OLD_KAKAO_VERSIONS --current-version 92 --current-path "%CD%"
goto DONE

:CANCEL
echo [v92] Cancelled. No files changed.
pause
exit /b 0

:DONE
echo.
echo [v92] Cleanup tool finished.
echo [v92] A JSON report was saved under outputs\cleanup_reports or the user-data cleanup report folder.
echo.
pause
exit /b %ERRORLEVEL%
